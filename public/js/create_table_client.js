				...
				function appendColumn() {
					...
					if ($("#column tr").length == 1) {
						...
						.append(col_template)
						.find("tr:last")
						.append('<td><button class= "remove button" /></td>');
						...
						for(var i = 1 ; i < $opt.length ; i ++){
							...
							$('<option>')
							.text($opt.eq(i).val()).val($opt.eq(i).val())
							.appendTo($tr.find('.type'));
				...
				$("#fTable").change(function(){
					...
					if($(this).find('option:selected').index()){
						...
						socket.emit('set_tabname_col', {scname: $("#fSchema").find('option:selected').text(), 
				...
				$("#fBtnBox button:eq(0)").click(function(e){//ok button
					...
					if($("#fColumn").find('option:selected').index() ){
				...
				function validCheck() {
					...
					for (var i = 1; i <= index; i++) {
						...
						if (!$type.children("option:selected").index() ){
						...
						if ($fkey.is(':checked') && !$fkey.siblings(".f.f_column").find('option:selected').index()){
							
							notValid($fkey.siblings('.f.f_column'));
